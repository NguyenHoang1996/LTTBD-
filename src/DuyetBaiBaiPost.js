...
class DuyetBaiBaiPost extends Component {
    ...
    render() {
        let test = this.state.aaaa.map(item => {
            let binhLuan, binhLuan2;
            let tenDiaChi;
            if (item.diaChi.length < 40)
                tenDiaChi = <Text style={{ fontSize: 12 }}>{item.diaChi}</Text>
            else
                tenDiaChi = <Text style={{ fontSize: 12 }}>{item.diaChi.slice(0, 40) + "..."}</Text>

            if (item.comment[0].userComment.length < LIMIT_CHARACTER)
                binhLuan = <Text style={{ fontSize: 12 }}>{item.comment[0].userName + " : " + item.comment[0].userComment}</Text>
            else
                binhLuan = <Text style={{ fontSize: 12 }}>{item.comment[0].userName + " : " + item.comment[0].userComment.slice(0, LIMIT_CHARACTER) + "..."}</Text>

            if (item.comment[1].userComment.length < LIMIT_CHARACTER)
                binhLuan2 = <Text style={{ fontSize: 12 }}>{item.comment[0].userName + " : " + item.comment[1].userComment}</Text>
            else
                binhLuan2 = <Text style={{ fontSize: 12 }}>{item.comment[0].userName + " : " + item.comment[1].userComment.slice(0, LIMIT_CHARACTER) + "..."}</Text>
            ...
            return (
                <View key={item.id}>
                        ...
                        <View style={{ height: HEIGHT_COL, width: "100%", flexDirection: "row" }}>
                            ...
                            <View style={{ flex: 1, flexDirection: "column", justifyContent: "center", marginRight: 8 }}>
                                ...
                                <View style={{ height: HEIGHT_COL / 2, flexDirection: "row", justifyContent: "space-between" }}>
                                    {/* <Text style={{ fontSize: 12 }}>{item.diaChi}</Text> */}
                                    {tenDiaChi}
                    ...
                    />
                    <Divider styleName="line" />
                    <TouchableOpacity onPress={() => this.toCommentChiTiet(item.comment[0].userId)}>
                        <View style={{ height: HEIGHT_COL, width: "100%", flexDirection: "row" }}>
                    ...
                    <Divider styleName="line" />
                    <TouchableOpacity onPress={() => this.toCommentChiTiet(item.comment[1].userId)}>
                        <View style={{ height: HEIGHT_COL, width: "100%", flexDirection: "row" }}>
                            <View style={{ height: HEIGHT_COL, width: HEIGHT_COL, justifyContent: "center" }}>
                                <Image
                                    style={{ height: HEIGHT_COL / 2, width: HEIGHT_COL / 2, marginLeft: 8 }}
                            ...
                            <View style={{ flex: 1, flexDirection: "column", justifyContent: "center" }}>
                                <View style={{ height: HEIGHT_COL, justifyContent: "center" }}>
                                    {/* <Text> {item.comment[1].userComment} </Text> */}
                                    {binhLuan2}
                                </View>
                            </View>
                        </View>
